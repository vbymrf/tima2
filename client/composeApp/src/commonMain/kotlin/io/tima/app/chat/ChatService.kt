...
data class ChatMessage(
    ...
    // Разметка (ADR-0011, Р5б) — компактный JSON, пусто у сообщения без оформления.
    val markup: String = "",
)
...
interface ChatClient {
    ...
    /**
     * Отдать историю всех своих чатов только что подключённому устройству
     * (привязка по QR, key-lifecycle.md §2). Возвращает число чатов, для которых
     * ключи удалось перезавернуть.
     *
     * Почему это делает подключающее устройство, а не подключённое: у нового
     * устройства нет ключа личности из секретной фразы, а `chatRecover` требует
     * подписать им запрос. Просить историю оно не может — значит, отдать её
     * должен тот, кто и так подтвердил привязку осознанным действием (сканом).
     * Согласие тут уже получено самим фактом скана, поэтому диалога нет.
     */
    suspend fun shareHistoryWithDevice(deviceId: String, deviceEncPub: String): Int
