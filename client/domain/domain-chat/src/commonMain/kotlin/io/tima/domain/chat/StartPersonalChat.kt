...
interface ChatFacts {
    ...
    /**
     * Когда мы в последний раз ответили в этой переписке системным сообщением; 0 — не
     * отвечали (Л14).
     *
     * Выводится из самой переписки, а не из отдельного хранилища «когда отвечали»:
     * устройств у человека несколько, и каждое завело бы своё. Отправленное приедет на
     * все эхом — то же правило, по которому список переписок выводится из сообщений.
     */
    fun lastSystemOutMs(chatId: String): Long = 0
}
