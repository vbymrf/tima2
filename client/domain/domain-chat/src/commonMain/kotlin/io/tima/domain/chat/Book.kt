...
class ObserveBook(private val book: Book) {
    ...
    /** Контакты: книга без убранных и заблокированных. Порядок и разделы — дело экрана. */
    fun list(): Flow<List<BookEntry>> = book.list()

    /**
     * Вся книга, включая оба списка — Л5.
     *
     * Отдельным потоком, а не фильтром поверх [list]: тот уже отобран в базе, и убранных
 ...
 *
 * @param phone номер в E.164; пусто у заведённого по нику. Ключом он быть перестал —
 *   им стал [BookEntry.id], см. `BookKey` (Л0).
...
interface Book {
    ...
    /**
     * Имя и значок «Общего». Строка заводится при первой правке — до неё «Общий»
     * называется словом из словаря. Убрать его нельзя: сюда попадают все, кому раздел не
     * выбран.
     */
