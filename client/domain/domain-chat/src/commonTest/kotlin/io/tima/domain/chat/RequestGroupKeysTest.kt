...
class RequestGroupKeysTest {
    ...
    fun просьба_ушла_помощникам() = runTest {
        val шаг = RequestGroupKeys { RecoveryStep.Requested(versions = 3, helpers = 2) }
            .запросить(группа)
    ...
    fun просить_некого_это_отдельный_исход() = runTest {
        ...
        val шаг = RequestGroupKeys { RecoveryStep.Requested(versions = 3, helpers = 0) }
            .запросить(группа)
    ...
    fun недостающих_версий_нет() = runTest {
        ...
        val шаг = RequestGroupKeys { RecoveryStep.Requested(versions = 0, helpers = 5) }
            .запросить(группа)
    ...
    fun нужна_секретная_фраза_а_не_ошибка() = runTest {
        ...
        val шаг = RequestGroupKeys { RecoveryStep.NeedsSecretPhrase }.запросить(группа)
    ...
    fun обрыв_и_отказ_проходят_наружу() = runTest {
        assertIs<RequestKeysStep.Offline>(
            RequestGroupKeys { RecoveryStep.Offline(2_000) }.запросить(группа),
        )
        val отказ = RequestGroupKeys { RecoveryStep.Refused("internal") }.запросить(группа)
