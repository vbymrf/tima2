...
class ОкружениеTest {
    ...
    fun второй_запуск_видит_написанное_в_первый() = runTest {
        val первое = Окружение.открыть(база(), СЕКРЕТ)
        assertTrue(первое.отправка.send("chat-1", "привет") is SendMessageResult.Queued)

        val второе = Окружение.открыть(база(), СЕКРЕТ)
    ...
    fun без_сети_сообщение_ждёт_в_очереди_а_не_исчезает() = runTest {
        val окружение = Окружение.открыть(база(), СЕКРЕТ)
    ...
    fun с_чужим_секретом_переписка_не_читается() = runTest {
        Окружение.открыть(база(), СЕКРЕТ).отправка.send("chat-1", "привет")

        val чужое = Окружение.открыть(база(), ЧУЖОЙ_СЕКРЕТ)
