...
class Entry private constructor(
    ...
    /**
     * Сколько у аккаунта неотправленного (Д11, путь Б, смягчение 2).
     *
     * Читается из хранилища платформы, а не из очереди: чужая очередь лежит в чужой базе
     * под чужим ключом покоя, и открыть её ради числа значило бы держать рядом ключи всех
     * аккаунтов — ровно то, чего путь Б и избегает.
     */
    fun pendingOf(userId: String): Int = accounts.pending(userId)

    /** Открытый аккаунт оставляет своё число списку: больше его записать некому. */
    fun notePending(userId: String, howMany: Int) = accounts.notePending(userId, howMany)
