...
kotlin {
    ...
    sourceSets {
        commonMain.dependencies {
            ...
            // Крипто-типы нужны ПРОВЕРКАМ: FullCircleTest и AndroidStandTest строят
            // escrow-метаданные и подписи напрямую — это их работа, они проверяют
            // сам протокол. Объявлено явно, а не взято через api-экспорт
            // core-encryption: иначе сужение его экспорта ломало бы прогон стенда.
