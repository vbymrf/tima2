...
interface CallWords {
     ...
     * знать, что нажимать дальше, а не что операция не удалась.
     */
    val noCamera: String
     ...
     *
     * **Не «недоступен» и не конец звонка.** Сервер знает ровно то, что подтверждения
     * кадра не пришло; появится устройство — заберёт вызов из журнала само. Надпись
...
object RussianWords : Words {
    ...
    override val bench = object : BenchWords {
        ...
        override val arm = "Начать прогон"
        override val disarm = "Остановить прогон"
        override val armAbout =
            "Забег идёт: каждый звонок берёт следующий набор, числа пишутся, файл ложится на телефон, " +
                "а в окне звонка видно номер прогона"
