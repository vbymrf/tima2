...
object EnglishWords : Words {
    ...
    override val appearance = object : AppearanceWords {
        ...
        override val slotNavigation = ColorSlotWords(
            name = "Navigation and action",
            about = "logo, current window, «back», «send»",
        )
        override val slotActivity = ColorSlotWords(
            name = "Activity",
            about = "unread counter",
        )
        override val slotConfirmed = ColorSlotWords(
            name = "Confirmed",
            about = "delivered, read, the E2E mark",
        )
        ...
        override val slotLine = ColorSlotWords(
            name = "List divider",
            about = "between entries",
        )
        override val slotOnAccent = ColorSlotWords(
            name = "Text on green",
            about = "on buttons, tabs, the header plate",
        )
        override val slotOnAmber = ColorSlotWords(
            name = "Text on amber",
            about = "on the unread counter",
        )
        override val slotInPlate = ColorSlotWords(
            name = "Inside the header plate",
            about = "logo and buttons on light green",
        )
        override val slotSoftAccent = ColorSlotWords(
            name = "Soft backing",
            about = "unselected tab, input field",
        )
        ...
        override fun colorNotHex(listed: String) =
            "Not hexadecimal: $listed. Allowed are 0–9 and A–F"
        override fun colorWrongLength(length: Int) =
            "$length characters, but 6 (colour) or 8 (with opacity) are needed"

        override val qrTooLong = "Cannot show the code"
        override val qrTooLongAbout = "It is too long for a QR code"
        override val theme = "Theme"
        override val themeKind = "Custom theme type"
        override val themeKindAbout = "sets the author stripe colours in groups"
        override val authorStrips = "Coloured author stripes"
        override val authorStripsAbout = "each member gets their own; off — outline colour for all"
    ...
    override val bench = object : BenchWords {
        ...
        override val onCharger = "charging"
        override val mahSpent = "Spent, mAh"
        override val currentAverage = "Current, average"
    ...
    override val windows = object : WindowWords {
        ...
        override val call = WindowName(
            full = "Call",
            short = "Call",
            about = "a call is in progress",
        )
        override val phone = WindowName(
            full = "Phone",
            short = "Phone",
            about = "chats, contacts, calls",
        )
        override val social = WindowName(
            full = "Social feed",
            short = "Social",
            about = "common, friends, catalogue",
        )
        override val media = WindowName(
            full = "Media feed",
            short = "Media",
            about = "feed and slides",
        )
        override val activity = WindowName(
            full = "Open conversation",
            short = "Talk",
            about = "stories, answers, reactions",
        )
    ...
    override val settings2 = object : SettingsListWords {
        ...
        override val groupApplication = "Application"
        override val groupBlogger = "Blogger"
        override val groupHelp = "Help"
        ...
        override val soundsTitle = "Sounds"
        override val soundRing = "Ringtone"
        override val soundMessage = "Notification sound"
        override val soundDefault = "As in the system"
        override val soundSilent = "Silent"
        override val soundFromSystem = "From standard"
        override val soundFromFile = "Load a file"
    ...
    override val social = object : SocialWords {
        ...
        override val nickField = "Nickname"
        override val fromContacts = "From contacts"
        override val noSuchNickname = "Nobody has this nickname"
        override val alreadyMember = "already in the group"
    ...
    override val book = object : BookWords {
        override fun inCard(count: Int) = "in card: $count"
        ...
        override val labelsTitle = "Section labels"
        override val labelsIcons = "Icons"
        override val labelsIconsAbout = "sections as glyphs"
        ...
        override val addSection = "Add a section"
        override val newSectionName = "Name"
        override val sectionIcon = "Icon"
        override val noIcon = "no icon"
        override val removeSection = "Remove section"
        override val removeSectionAbout = "its people go back to “General”"
        override val sectionsEmpty = "No sections yet"
        override val sectionsEmptyAbout = "A section is a shelf for contacts: “Work”, “Home”, “Study”. Create the first one"
        override val sectionsScreenCommunity = "Community sections"
        override val sectionsEmptyAboutCommunity =
            "A section is a shelf for groups and channels: “Work”, “Neighbours”, “Study”. Create the first one"
        override val removeSectionAboutCommunity = "its groups go back to “General”"
        ...
        override val ledgerSelectNone = "Clear selection"
        override val ledgerSearch = "Name or number"
        override val ledgerToSection = "To section"
        override val ledgerRemove = "Remove"
        override val ledgerBlock = "Block"
        override val ledgerRestore = "Restore"
        override val ledgerSound = "Sound"
        override val ledgerPickSection = "Where to put the selected"
        override val ledgerSoundTitle = "Ringtone for the selected"
        override val ledgerSoundAsSettings = "As in settings"
        override val ledgerSoundOnlyTima = "Ringtone only for people in TIMa: others do not call you through the app"
        override fun ledgerSelected(count: Int) = "Selected: $count"
    ...
    override val chat = object : ChatWords {
        ...
        override val findByNickname = "Find"
        override val nobodyWithNickname = "Nobody has that nickname"
        ...
        override val myColor = "My colour in the group"
        override val myColorAbout = "the stripe on my messages as others see it"
        override val myColorAuto = "automatic"
        override val myColorTaken = "taken"
        override val myColorReset = "Reset — automatic"
        override val notSent = "Not sent"
        override val waitingTitle = "Waiting to send"
        override val waitingNoReason = "Waiting in the queue — no attempts yet"
        override fun waitingAbout(attempts: Int, seconds: Int): String {
            val tries = if (attempts == 0) "no attempts yet" else "attempts: $attempts"
            val next = when {
                seconds <= 0 -> "next one on the nearest pass"
                seconds < 60 -> "next one in $seconds s"
                else -> "next one in ${seconds / 60} min"
            }
            return "$tries · $next"
        }
        ...
        override fun notInTimaChecked(phone: String) =
            "TIMa has no $phone. The contact will be saved — you can call by phone. " +
                "If the person is on TIMa, check the number"
        ...
        override val avatarRotate = "Rotate"
        override val pickSection = "Pick a section"
        override fun createSectionNamed(name: String) = "Create section “$name”"
        override fun noSuchSection(name: String) =
            "There is no “$name” section. Create it or pick one from the list — otherwise " +
                "the contact lands where nobody can see it"
    ...
    override val wizard = object : WizardWords {
        override val create = "Create"
        override val creating = "Creating…"
        override val goToCreated = "Open the group"
        override val shelf = "Section"
        override val shelfAbout = "where to put it: move later via “•••”"
        override val shelfCommon = "General"
        override val newShelfHint = "new section"
        override val createShelf = "Create"
    ...
    override val tabs = object : TabWords {
        ...
        override val calls = "Calls"
        override val view = "View"
        override val all = "All"
        override val fromBook = "In contacts"
