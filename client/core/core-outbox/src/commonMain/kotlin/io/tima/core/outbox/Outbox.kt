...
class Outbox(
     ...
     *
     * **Зовёт это `shared/Assembly.kt`, при сборке аккаунта** — и названо здесь потому,
     * что полтора месяца не звал никто, кроме тестового стенда. Метод был написан,
     * покрыт проверками и мёртв: сообщение, застрявшее в `SENDING`, стояло в очереди
     * пять суток (realme 2026-09-23). Проверка на месте вызова невозможна — он в
     * Compose-сборке; значит остаётся эта строка.
     *
    ...
    /**
     * @return `false` — результат **опоздал**: записи в `SENDING` уже нет, применять его
     *   не к чему.
     */
    fun onOutcome(dedupKey: String, outcome: SendOutcome): Boolean {
        val entry = store.byDedupKey(dedupKey)
            ?: error("нет записи $dedupKey: результат пришёл не на своё сообщение")
        // ── ОПОЗДАВШИЙ РЕЗУЛЬТАТ — НЕ ОШИБКА КОДА ───────────────────────────
        //
        // Здесь стоял `require`, и он ронял **всё приложение**. Стоило этого на realme
        // 2026-09-23: восстановление очереди вернуло запись из `SENDING` в `QUEUED`,
        // пока отправитель ходил за ключом группы, — и приложение закрылось само на
        // первом же запуске, в первый же день, когда восстановление доехало до телефона.
        //
        // Корень той беды починен в `Assembly`: восстановление зовётся раз на процесс.
        // Но `require` здесь неверен и сам по себе — состояние записи меняют и
        // восстановление, и человек («Удалить неотправленное», «Повторить»). Гонка с
        // ними возможна всегда, и она **не ошибка программиста**.
        //
        // Уронить приложение, защищая запись в очереди, — обмен худший из возможных:
        // теряется всё, что человек делал, ради одной строки, которую можно просто не
        // писать.
