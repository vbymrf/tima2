...
interface InboxStore {
     ...
     *
     * Пачкой и в SQL, а не по одному через машину: переписка открывается целиком, и
     * тысяча вызовов вместо одного запроса — это тысяча транзакций. Предусловие при этом
     * то же самое и живёт в запросе: **только из `STORED`**. Непрочитанным остаётся то,
     * что человек ещё не мог прочитать — неразобранное и нечитаемое.
     *
     * @return сколько строк перешло.
     */
...
class Inbox(
     ...
     *
     * Тот же переход, что у [markRead], но пачкой — и с тем же предусловием: из `STORED` и
     * только из него. Неразобранное и нечитаемое прочитанным не становится: человек его не
     * читал и, возможно, ещё прочитает.
     */
    fun markChatRead(chatId: String): Int = store.markChatRead(chatId)
}
