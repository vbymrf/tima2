...
class OutboxTest {
    ...
    /**
     * Результат на запись, которая уже не в `SENDING`, **не применяется** — и не роняет
     * приложение.
     *
     * Раньше здесь падало исключение, и падало оно до самого верха: 2026-09-23 на realme
     * приложение закрывалось само на первом же запуске. Восстановление очереди вернуло
     * запись из `SENDING` в `QUEUED`, пока отправитель ходил за ключом группы, — и
     * отправитель вернулся с результатом на запись, которой в `SENDING` уже не было.
     *
     * Гонка тут возможна всегда: состояние меняют и восстановление, и человек
     * («Удалить неотправленное», «Повторить»). Это **не ошибка программиста**, и ронять
     * ради неё всё приложение — обмен худший из возможных.
     */
    @Test
    fun опоздавший_результат_не_применяется_и_не_роняет() {
        put()
        outbox.sealNext(1, seal) // SEALED, но не SENDING
        assertFalse(
            outbox.onOutcome("dedup-1", SendOutcome.Accepted(1)),
            "результат применился к записи, которая не отправлялась",
        )
        // Главное: запись осталась какой была. Принять результат значило бы отметить
        // отправленным то, что не отправляли.
        assertEquals(OutboxState.SEALED, store.byDedupKey("dedup-1")?.state)
    }
