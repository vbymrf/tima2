 ...
 * Тесты Kotlin/Native — отдельный исполняемый файл, а не приложение: у него нет права
 * `application-identifier`, и Keychain отвечает `errSecMissingEntitlement` (-34018) на
 * любое обращение. Настоящая проверка Keychain требует прогона **внутри приложения**
 * (`app-ios`, К5) — это ровно тот же случай, что у Android, где AndroidKeyStore
...
class KeychainVaultTest {
    ...
    /** Доступен ли Keychain этому процессу. Выясняется одним обращением, а не догадкой. */
    private val доступен: Boolean = try {
        KeychainVault(служба).remove(SecretAlias("проба.v1"))
        true
    ...
    private fun проверитьВнятныйОтказ() {
        ...
        // И чтение ведёт себя так же: молчаливого «секрета нет» быть не должно, иначе
        // приложение решило бы, что это первый запуск, и завело второе устройство.
        assertFailsWith<KeychainUnavailable> { vault.get(имя) }
    }
