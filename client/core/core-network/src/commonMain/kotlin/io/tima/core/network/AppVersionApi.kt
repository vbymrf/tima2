...
class AppVersionApi(
    ...
    suspend fun latest(platform: String = ""): AppVersionResult {
        ...
        return AppVersionResult.Version(
            ...
            // Важность появилась 2026-09-06 и тоже необязательна. Отсутствие поля —
            // «обычное обновление»: сервер, который о важности не знает, не должен
            // случайно поднимать окно при каждом запуске у всех.
            important = body.bool("important") ?: false,
        )
    }
}
...
sealed interface AppVersionResult {
    data class Version(
        ...
        val minClient: Int = 0,
        /** Важное обновление: окно при каждом запуске, но закрывается. */
