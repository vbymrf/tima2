...
fun timaHttpClientWithChannel(
    ...
    install(WebSockets) {
        // ── СВОЙ ПИНГ, И ЭТО ОБРАТНОЕ ПРЕЖНЕМУ РЕШЕНИЮ ──────────────────────
        //
        // В `EventStream` записано: «Ping/pong не наша забота: сервер пингует сам, клиент
        // Ktor отвечает автоматически. Своего пинга не добавляем — два пинга навстречу
        // друг другу только жгут батарею». Довод про батарею верен; неверна посылка, что
        // чужого пинга достаточно.
        //
        // **Достаточно его только при живом сокете.** Телефон ушёл с Wi-Fi на мобильную,
        // NAT забыл отображение — и соединение становится ПОЛУОТКРЫТЫМ: сервер своё
        // давно закрыл по таймауту pong, а клиент сидит в `for (frame in incoming)`
        // вечно. Закрытия не будет, потому что закрывать некому: пакета RST никто не
        // слал.
        //
        // Это измерено, а не предположено: realme 2026-09-23 два дня не получал ни
        // одного события. Последнее подтверждение — 09-21 09:33, канал поднят 09-21
        // 07:44, за двое суток в журнале **ни одной попытки** переподключиться. HTTP при
        // этом работал: звонки заводились, ключи читались. Телефон выглядел живым и был
        // глухим — входящий звонок к нему было не доставить ничем.
        //
        // Ktor закрывает сессию, если на свой пинг не дождался pong. Закрытие и есть то,
        // чего не хватало: `run` возвращает исход, `hold` ждёт две секунды и подключается
        // заново.
        //
        // ── ИНТЕРВАЛ ВЫВЕДЕН ИЗ ВРЕМЕНИ ЗВОНКА, А НЕ ИЗ ЭКОНОМИИ (У13) ──────
        //
        // Движок закрывает сессию не сразу, как pong не пришёл, а когда подошёл срок
        // СЛЕДУЮЩЕГО пинга. Значит слепое окно — до **двух интервалов**, а не одного.
        //
        // А вызов звонит сорок пять секунд (константа продукта, на неё ссылаются восемь
        // мест сервера). Слепое окно длиннее звонка означает, что вызов, пришедший
        // сразу после тихой смерти сокета, пропадает ЦЕЛИКОМ — и в журнале выглядит
        // как `missed`, то есть «не взял трубку», хотя телефон не звонил.
        //
        //     2 × интервал  +  подъём канала  <  45 с
        //     2 × 18 с      +  ~3 с           =  39 с
        //
        // Было 25 с — то есть до 53 с в худшем случае, длиннее звонка. Считать это
        // «запасом по батарее» нельзя: запас был за счёт звонков.
        //
        // **Поднимать обратно нельзя**, и вот чем это отличается от Telegram, у
        // которого пинг раз в 60 с: у него входящий звонок будит телефон ПУШЕМ, а не
        // сокетом. У нас пуша пока нет (ПЛАН-ПУШ-УВЕДОМЛЕНИЙ), и сокет — единственный
        // звонок в дверь.
