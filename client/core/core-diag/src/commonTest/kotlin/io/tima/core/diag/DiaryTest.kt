...
class DiaryTest {
    ...
    fun разговорчивый_цикл_не_вытесняет_всё() {
        ...
        //
        // ── ПРОВЕРКА ИЗМЕНИЛАСЬ 2026-09-17 ВМЕСТЕ С ПОВЕДЕНИЕМ ──────────────────────
        //
        // Раньше здесь требовалось, чтобы последней осталась «попытка 99»: предел в
        // памяти работает, свежее вытесняет старое. Гарантия была слабее, чем казалось —
        // сам цикл всё равно вытеснял из журнала ВСЁ остальное, включая то, из-за чего он
        // начался. На настоящей беде (`БЕДЫ/2026-09-17`) это и вышло: круг шёл минутами,
        // и к моменту разбора в журнале не осталось ничего, кроме круга.
        //
        // Теперь цикл называется кодом LOOP-SUSPECT и дальше не пишется до конца всплеска.
        // Требование стало сильнее: журнал не только переживает цикл, но и говорит, какой
        // код в нём крутился.
        ...
        val last = notes.last()
        assertEquals(LogCode.LOOP_SUSPECT, last.code, "цикл обязан быть назван")
        assertTrue(
            last.details.any { it.first == "код" && it.second == LogCode.NET_CALL },
            "названный цикл обязан указывать, какой код крутился: без этого строка ни о чём",
        )
        assertTrue(
            notes.any { it.code == LogCode.NET_CALL },
            "записи цикла обязаны остаться до порога: по ним видно, с чего он начался",
        )
    }
