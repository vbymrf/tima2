...
class PersonalMessages(escrowKey: EscrowEpochKey) {
    ...
    companion object {
        /**
         * **Непроверенная подсказка из открытой части конверта: кто его прислал.**
         *
         * Нужна ровно для одного — понять, чей ключ подписи спрашивать у сервера,
         * прежде чем открывать сообщение. Подписью это не подтверждено: открытая
         * часть конверта не защищена, и назваться там можно кем угодно.
         *
         * Отправитель становится доверенным только после [open], где подпись
         * проверяется по ключу, полученному по этой подсказке. Если подсказка была
         * ложной — ключ не подойдёт, и сообщение не откроется; поэтому подсказке
         * можно верить в выборе ключа и нельзя — ни в чём другом.
         *
         * Приёмник до 2026-08-25 звал `MessageSerializer.decodeEnvelope` сам, ради
         * этих двух полей таща крипто-типы в composition root.
         *
...
/**
 * Кто назвался отправителем в открытой части конверта.
 *
 * Отдельный тип, а не пара строк, ровно по одной причине: его имя говорит, что
