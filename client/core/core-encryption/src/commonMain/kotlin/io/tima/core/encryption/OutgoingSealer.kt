...
class OutgoingSealer(
    ...
    companion object {
         ...
         * little-endian.
         *
         * Шестьдесят четыре бита случайного UUID — достаточная развязка для чисел,
         * которые сравниваются только внутри одной переписки. Главное свойство здесь
         * не уникальность, а **устойчивость**: один и тот же `dedup_key` обязан давать
         * одно и то же число на каждом повторе.
         */
