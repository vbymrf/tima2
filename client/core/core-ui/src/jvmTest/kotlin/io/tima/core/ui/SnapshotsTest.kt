...
class SnapshotsTest {
     ...
     * салатовом — **белый в обеих темах**, 2,08 : 1, ниже порога 4,5 и принято
     * осознанно. Так же в макете: `#ffffff` у `.имя-окна`, `.таб.тек`, `.чип.выбран`,
     * `.икона.жив`.
     *
     * Прежняя редакция теста требовала обратного для светлой темы — чёрный, 10,08 : 1
     * (решение 2026-08-23, довод — мелкие подписи). Переворот здесь намеренный и
     * симметричный: тест по-прежнему покраснеет, если цвет уедет молча, только теперь
     * в другую сторону.
     */
    @Test
    fun текст_на_салатовой_кнопке_белый_в_обеих_темах() {
        ...
        for ((name, snapshot) in snapshots) {
            ...
            val ratio = TimaContrast.ratio(glyph, theme(name).navigation)
            assertTrue(
                ratio < TimaContrast.TEXT_THRESHOLD,
                "$name: текст на салатовом стал читаемым ($ratio) — решение было " +
                    "«белый, 2,08», и менять его молча нельзя",
            )
            assertTrue(
