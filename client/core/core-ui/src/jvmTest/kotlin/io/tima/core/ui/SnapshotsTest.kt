...
class SnapshotsTest {
    ...
    /**
     * Аватар стоит **за скруглением** пузыря (проба «аватар-за-округлением», принята
     * заказчиком 2026-09-19): полосу автора и угол рамки он не перекрывает — они видны
     * целиком, а сам он выступает вверх, в зазор между репликами.
     *
     * До этого правило было обратным: аватар на краю, поверх полосы. Проверка сравнивает две
     * сцены, а не координату с числом: начало полосы с аватаром и без него обязано совпасть.
     */
    @Test
    fun аватар_за_скруглением_полосу_не_перекрывает() {
        ...
        for (name in listOf("светлая", "тёмная")) {
            ...
            val withIt = firstGreen(withAvatar.getValue(name), colors.navigation)
            assertTrue(
                withIt == top,
                "$name: полоса с аватаром начинается на y=$withIt против y=$top без него — " +
                    "аватар снова лёг на полосу, а по пробе он стоит за скруглением",
            )
        }
    }
    ...
    /**
     * Решение заказчика от 2026-09-02, записанное пикселями, а не словами: текст на
     * заливке навигации — **белый в обеих темах**. Так же в макете: `#ffffff` у
     * `.имя-окна`, `.таб.тек`, `.чип.выбран`, `.икона.жив`.
     *
     * **Цена белого перестала быть одинаковой 2026-09-03**, когда тёмная тема получила
     * свой тёмно-зелёный. В светлой это по-прежнему 2,08 : 1 — ниже порога 4,5 и принято
     * осознанно; в тёмной стало 11,46, то есть решение там больше ничего не стоит.
     * Поэтому проверяется не «нечитаемо», а **белизна самой надписи**: она и есть
     * решение. Числа контраста держит `TokensTest`, и держит порознь по темам — иначе
     * общая проверка врала бы про обе.
     */
    @Test
    fun текст_на_заливке_навигации_белый_в_обеих_темах() {
        val snapshots = bothThemes("кнопка", 160, 60) {
            Box(Modifier.padding(9.dp)) { Button("Отправить", onClick = {}) }
        }
        for ((name, snapshot) in snapshots) {
            val glyph = snapshot.lightMost(x = 30..130, y = 20..40)
            assertTrue(
                Snapshot.close(glyph, theme(name).onAccent),
                "$name: самая светлая точка надписи — $glyph, а обязана быть белой",
            )
        }
    }
