...
class ArchitectureTest {
    ...
    /**
     * То же доказательство для шести правил: крипто-фасад, SQLDelight, Ktor и два
    ...
    fun правила_шага_1_ловят_нарушения() {
        val waiting = mapOf(
            ...
            // Словарь: перечень оболочки уезжает под него сначала параметром, потом
            // целиком — так это уже случилось однажды (ПЛАН-ЯЗЫКА, Я-D).
            "core-words без зависимостей" to "io.tima.feature.shell",
        )
    ...
    /**
     * **Гарантия очереди обязана быть подключена, а не только написана.**
     *
     * `Outbox.recoverOnStart` возвращает в очередь то, что застряло в `SENDING` и
     * `SEALED`, когда Android убил приложение посреди отправки. Метод был написан,
     * покрыт проверками — и **полтора месяца его не звал никто, кроме тестового
     * стенда**. Сообщение на realme простояло в очереди пять суток, а журнал каждые пять
     * секунд писал «повтор не помог», не называя причины (2026-09-23).
     *
     * В самом `Outbox` про это сказано прямо: в v1 такое сообщение «пропадало без следа
     * для человека». v2 повторила это в точности — не ошибкой в машине, а отсутствием
     * строки вызова.
     *
     * Проверять вызов на месте нельзя: он в Compose-сборке. Значит проверяется наличие
     * вызова **в рабочем коде**, а не в проверках, — и этого достаточно, чтобы его не
     * потеряли снова при переносе сборки.
     */
    @Test
    fun восстановление_очереди_подключено_к_рабочему_коду() {
        val callers = clientRoot.walkTopDown()
            .onEnter { it.name !in setOf("build", ".gradle", ".kotlin", ".git") }
            .filter { it.isFile && it.extension == "kt" }
            // Проверки и стенды не в счёт: они звали его и тогда, когда приложение не звало.
            .filter { file -> SOURCE_SETS_OF_TESTS.none { file.path.contains(it) } }
            // Само объявление метода — не вызов.
            .filter { it.name != "Outbox.kt" }
            .filter { it.readText().contains("recoverOnStart()") }
            .map { it.name }
            .toList()

        assertTrue(
            callers.isNotEmpty(),
            "recoverOnStart() не зовёт никто из рабочего кода: сообщение, застрявшее в " +
                "SENDING при убийстве приложения, не уйдёт никогда, а человек будет видеть «ждёт»",
        )
    }
    ...
    private companion object {
        /**
         * Куски пути, по которым узнаются проверки и стенды.
         *
         * Без разделителей каталогов: на Windows он обратный, и путь с прямыми слэшами
         * там не совпадёт ни с чем — проверка тихо пропустила бы всё.
         */
        val SOURCE_SETS_OF_TESTS = listOf("commonTest", "jvmTest", "test-harness", "test-ui")
    }
}
