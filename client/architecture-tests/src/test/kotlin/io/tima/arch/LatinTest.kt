 ...
 * них.
 *
 * **Имена ВНУТРИ кода проверяет не этот тест, а хук** `.claude/hooks/latin_names.py`
 * (решение заказчика 2026-09-24) — на каждую правку агента. Сюда эта проверка не
 * переезжает и не дублируется. Нужна она оказалась потому, что этот тест содержимого не
 * видит: за месяц после перехода 2026-08-26 в коде набралось 37 кириллических имён, и
 * никто не заметил. Тесты хук не проверяет вовсе — тем же решением.
 *
