 ...
 * же, поэтому переключатель стоит рядом с вкладками, а не среди них, и сохраняется
 * при их смене.
 *
 * В макете он живёт **в шапке** и набран знаками «▤ ▣»: подписи «Лента | Слайды»
 * стоят 140 точек и вместе с логотипом, именем и двумя кнопками не помещаются в 380.
 * Здесь он опущен во второй ряд — место законное (`§1`: «под вкладками живут режимы,
 * фильтры или полоса разделов»), и причина в решении заказчика от 2026-09-02:
 * имя окна встало **ровно по центру** плашки, а центру нужен симметричный проём по
 * широкой стороне. С переключателем в шапке имени осталось бы 28 точек из 336 —
 * меньше, чем занимает слово «Медиа». Во втором ряду места хватает, поэтому вернулись
 * и подписи: знаки экономили ширину, которой здесь ничто не угрожает.
 */
@Composable
fun MediaWindow(
    onSwitchWindows: () -> Unit,
    ...
    WindowWithTabs(
        ...
        onNeighbourWindow = onNeighbourWindow,
        modifier = modifier,
        // Ряд без чипов: у окна 3 во втором ряду только режим. Пустой список слева —
        // не заглушка, а честная разметка макета, где режимы прижаты к правому краю.
        secondRow = {
            FilterRow(trailing = { ModeSwitch(MEDIA_MODES, mode, { mode = it }) })
        },
    ) { tab ->
