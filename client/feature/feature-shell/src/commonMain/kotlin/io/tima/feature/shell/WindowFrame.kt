...
fun WindowFrame(
    ...
    Column(modifier.fillMaxSize().background(colors.surface)) {
        ...
        // Идущий звонок виден из любого окна — иначе окно 0, будучи временным, теряется:
        // ушёл свайпом в «Чаты» и не знаешь, разговор ещё идёт или уже нет (ЗВ12).
 ...
 *
 * **Все вкладки стоят в одну строку** — решение заказчика 2026-09-03. Держится это не
 * запретом переноса, а зазором и полем вкладки: 6 вместо 8 и 11 вместо 14 ([Tab]).
 * Ряд из четырёх вкладок окна 5 просил 358 точек при 348 доступных, теперь просит 328.
 *
 * **Перенос при этом не запрещён, а оставлен на крайний случай.** [FlowRow] переносит
 * только то, что иначе не влезло бы; запрет вместо переноса означал бы обрезанное слово
 * у края, а это хуже второй строки. Что ряды в неё не уходят — проверяет [RowFitTest]
 * измерением, а не обещанием в комментарии.
 */
