 ...
 */
@Composable
private fun Лента(строки: List<ChatLine>, собеседник: String, modifier: Modifier = Modifier) =
    LazyColumn(
...
private fun Реплика(строка: ChatLine, автор: String, продолжение: Boolean) = Пузырь(
    ...
    низ = {
        Третьестепенное(время(строка.atMs))
        отметка(строка.display)?.let { Отметка(it) }
    },
) {
    val текст = строка.text
    if (текст != null) {
        Подпись(текст, кегль = TimaType.щ4)
    } else {
        // Нечитаемое сообщение остаётся в переписке: человек должен видеть, что
        // сообщение было. Иначе он ждёт ответа на то, чего собеседник, по его
        // сведениям, не присылал. Чип, а не текст: это не чужие слова.
        Чип("сообщение не читается", вид = ВидЧипа.Тихий)
    }
}
