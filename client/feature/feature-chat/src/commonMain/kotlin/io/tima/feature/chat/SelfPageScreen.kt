 ...
 *
 * Про срок — одна фраза, как реализовано на сервере: временный удаляется через 3 месяца
 * после создания (заказчик того же дня). Отметки активности у временного нет, поэтому
 * «после создания», а не «без входа». Сервер о состоянии не спрашивается: постоянным
 * аккаунт пока не становится никогда, и фраза верна для всех.
 *
