...
## Открыто: `markup_json` (ADR-0011, Р5б)

Раскладка узлов и разметки (`messenger-crypto/Markup.kt`, `MarkupTypes.kt`) пока
проверена только round-trip-тестами внутри Kotlin (`MessageContentTest.kt`,
`MarkupTypesTest.kt`) — decode(encode(x)) == x и деградация неизвестных типов до
`UNKNOWN`. Это не то же самое, что вектор здесь: `kotlinx.serialization` с
`encodeDefaults=false` опускает поля, равные значению по умолчанию (например,
`version=1`), и тонкости этого поведения нельзя надёжно посчитать вручную без
прогона — в отличие от `message_body` выше, значение `protobuf_hex` для примера с
`nodes`/`markup` здесь намеренно не выдумано.

Процедура заморозки — та же, что и у `message_body`: добавить фикстуру в
`SerializerTest.kt` (или отдельный `MarkupSerializerTest.kt`), взять реальный вывод
первого зелёного прогона, перенести константой в `vectors.json`. У Go-стороны пока
нет типизированного разбора `Markup` (сервер видит непрозрачные байты в приватном
контуре и просто проверяет валидность JSON в публичном — `internal/api/channels.go`),
поэтому вектор до появления такого разбора проверял бы только Kotlin сам с собой;
ценность вырастет, когда у Go появится читатель формата.
