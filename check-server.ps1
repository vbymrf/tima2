...
.DESCRIPTION
    ...
    печатает `ok`, пропустив 70 тестов из 118: отсутствие окружения выглядит как
...
if ($run -lt 100) {
    Note ("Выполнено " + $run + " тестов. Ожидание тира server-full — 118. Меньше значит, что часть не дошла до запуска.")
}
