...
> ⚠️ **Поправка 2026-07-30 — читать до остального текста.**
>
> Ниже по документу решения помечены как принятые со ссылками на **ADR-0016** и
> **ADR-0017**. **Этих ADR не существует.** Нумерация в `doc/adr/` заканчивается на
> 0010; следующий свободный номер — **0011**.
>
> Соответственно все пометки «✅ Решено ADR-0016 / ADR-0017» и ссылки на
> `security-test-plan.md`, `client-hardening.md`, `threat-model.md` следует читать как
> **предложения, а не решения**. Ни один из перечисленных документов в репозитории
> не заведён.
>
> Что из этого действительно принято к работе (обсуждение 2026-07-30):
>
> | Пункт аудита | Статус |
> |---|---|
> | R-1, key-commitment | **Принято к реализации.** Конверт версии 2, сверка на всех путях |
> | R-2, malleability подписи | **Принято**: отказ от неканонического `S` на приёме |
> | R-0, радиус escrow | **Принято, но иначе**: ключ на пару «чат × месяц» с ленивой генерацией, а не region×epoch×shard-бакеты |
> | R-3, length-extension | Принято как правило code-review, отдельной работы не требует |
> | R-4, side-channel в JS | Не актуально: браузерный клиент вне области |
>
> ADR на принятые решения нумеруются с 0011.
