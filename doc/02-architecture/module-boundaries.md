...
## 5. Формат контента постов: `nodes` + `markup`

> **ОТМЕНЕНО и заменено · сверено 2026-08-21.** Ниже сохранено прежнее решение от
> 2026-07-12 — «текст + массив entities» в стиле Telegram, — но оно **не
> действует**: его отменил [ADR-0011](../adr/0011-message-content-model.md).
> Действующая модель — **`nodes` + `markup`**: массив узлов и разметка, ссылающаяся
> на них по идентификаторам, а не по смещениям в строке.
>
> **Почему смещения не подошли.** Они привязывают разметку к конкретной раскладке
> символов: правка одного слова сдвигает всё, что после, и любая независимая правка
> двух устройств даёт разъехавшуюся разметку. Идентификаторы узлов от этого свободны.
>
> **Это не проект, а действующая схема:** `channel_posts` уже несёт столбцы `nodes
> TEXT[]`, `markup JSONB` и `markup_version INT` (миграция, добавившая их, заодно
> перенесла существующие посты одним узлом). Разбор и сборка разметки живут в
> `messenger-crypto`: `Markup.kt`, `MarkupTypes.kt`, `MessageContent.kt`.
>
> Прежний текст оставлен нарочно: удалять историю решений нельзя, устаревшее
> перекрывается новым со ссылкой. Читать его как проектный — **нельзя**: редактор,
> написанный по нему, будет несовместим с сервером.

Прежнее решение (обсуждено 2026-07-12, **отменено**): контент постов — **текст + массив entities** (Telegram-style), не Markdown и не JSON-блоки.
