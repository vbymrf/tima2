...
# Приложение. Сверка с миграциями · 2026-08-21

Выход задачи Д3. Сверялось с `server/migrations/*.sql` — 30 файлов.

## Объём

```
таблиц объявлено в этом документе:   64
таблиц существует в схеме:           27
есть и там и там:                    14
есть в схеме, здесь НЕ описано:      13
описано здесь, в схеме нет:          50
```

Пятьдесят неописанных в коде — это законно: документ проектный, и социальный слой в
нём спроектирован вперёд. **Тринадцать наоборот — нет.** Живая таблица, о которой
модель данных молчит, читается как «её не должно быть», и клиент на К3 построит схему
без неё.

## Тринадцать таблиц, которых здесь нет, а в схеме есть

| Таблица | Что держит | Почему это важно для К3 |
|---|---|---|
| `chat_states` | `archived_at` по паре «чат + человек» | сюда же поедет курсор прочтения (решение А2) |
| `sync_cursors` | курсор устройства для догрузки | без неё непонятно, как устроен офлайн |
| `device_events` | журнал доставки, монотонный `event_id` | по нему работают `/ws` и `sync.pull` |
| `escrow_keys` | реестр ключей эпох; **в документе он назван `escrow_periods`** | расхождение имён, не отсутствие |
| `personal_message_backup` | ключи сообщений под `backup_key` владельца | единственный путь восстановления истории на новом устройстве |
| `group_wrapped_keys` | групповой ключ, обёрнутый на устройство | вместе с `group_key_history` — вся крипто-часть групп |
| `channel_posts` | посты каналов, уже с `nodes`/`markup` | ADR-0011 в схеме **уже** есть |
| `channel_subscriptions` | подписчики канала | в документе подписки полиморфные (`subscriptions`), в схеме — отдельная таблица |
| `call_participants` | состояния `invited`/`joined`/`left` | групповые звонки |
| `voice_speakers` | право говорить в комнате | — |
| `sms_codes` | коды входа | — |
| `retention_policy` | сроки хранения строками, правятся SQL | сюда переехали сроки уборки (миграция 0030) |
| `gc_state` | водяной знак уборки событий | по нему `sync` отвечает `sync.gap` |

## Расхождения устройства, а не объёма

**1. Таблицы `chats` не существует.** Документ объявляет её (§3), а в схеме её нет:
чат опознаётся по `chat_id` внутри `personal_messages`, а состав участников
**выводится запросом** по `personal_message_keys` (`store.go`, `ChatHelperDevices`).
Следствие, которое надо знать: список получателей живёт 90 дней и вычищается уборкой
— то есть «участники чата» это не постоянная запись, а производная от ключей.

**2. `message_receipts` нет.** Прочтение не сохраняется вовсе: `POST /chats/{id}/read`
только рассылает событие. Это тот же вывод, что в решении А2, и он тут подтверждается
со стороны схемы.

**3. `contacts` нет, и это решение, а не пробел.** Адресная книга остаётся на клиенте
(решение А3): правило «присутствие видят только контакты» потребовало бы выложить граф
знакомств на сервер.

**4. `sessions` нет.** Сессия — это JWT плюс живая запись в `devices`; отдельной
таблицы под неё не заводилось.

## Столбцы: проверено точечно, а не целиком

**Сверка столбцов по всем четырнадцати общим таблицам здесь НЕ сделана** — говорю
прямо, чтобы приложение не читалось как полный аудит. Проверены выборочно те, что
нужны на К3; найденное ниже доказуемо.

**`persons`: документ называет столбец `account_state`, схема — `state`.** В
миграциях `account_state` не встречается ни раза (`0021_account_states.sql`
добавляет именно `state`).

Из-за этого **SQL в документе неисполним**. Здесь объявлено:
...
Скопировавший блок из документа получит ошибку про несуществующий столбец.

**`persons.credentials_due` в документе отсутствует.** В схеме он есть и несёт срок:
до этого момента можно входить без телефона и почты, после — только привязав их.

## Как перепроверить

```bash
# таблицы: документ против схемы
grep -ohiE 'create table( if not exists)? +[a-z_]+' doc/02-architecture/data-model.md \
  | sed -E 's/.* //' | sort -u > /tmp/doc.txt
grep -rhoiE 'create table( if not exists)? +[a-z_]+' server/migrations/*.sql \
  | sed -E 's/.* //' | sort -u > /tmp/code.txt
comm -13 /tmp/doc.txt /tmp/code.txt   # живое, но не описанное
comm -23 /tmp/doc.txt /tmp/code.txt   # описанное, но не живое

# конкретный столбец
grep -rn account_state server/migrations/   # пусто — значит имя другое
```

Первая пара команд и дала числа выше. Расхождение ловится их повторным запуском, а не
внимательностью читателя.
