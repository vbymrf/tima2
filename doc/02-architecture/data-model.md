...
-- апгрейд до full = привязка phone+email (key-lifecycle.md §8)
...
CREATE TABLE personal_message_keys (
    message_id      BIGINT NOT NULL,
    recipient_device UUID NOT NULL REFERENCES devices,
    wrapped_key     BYTEA NOT NULL,                -- Box(ephemeral, device_identity, message_key)
    PRIMARY KEY (message_id, recipient_device)
);
CREATE INDEX idx_pmk_recipient ON personal_message_keys (recipient_device, message_id DESC);
...
CREATE TABLE media_objects (
    ...
    content_hash    BYTEA,                         -- SHA-256 plaintext; NULL для приватного (opt-in CAS)
...
CREATE TABLE stories (
    ...
    is_encrypted    BOOLEAN NOT NULL,              -- личные истории шифруются
...
CREATE TABLE inbox_events (                        -- личные события агрегатора
    ...
    event_type      TEXT NOT NULL,                   -- reply|mention|reaction|comment|role_assigned|moderation_request
