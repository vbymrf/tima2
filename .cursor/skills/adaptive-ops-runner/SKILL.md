---
name: adaptive-ops-runner
description: Safely verifies, starts, diagnoses, and repairs the TIMA Windows development environment. Delegates uncertain operations to an execution worker, requires explicit approval for destructive or privileged actions, verifies each claimed state, and accumulates operational failure knowledge. Use automatically for environment setup, service failures, tooling installation, emulator problems, local runtime troubleshooting, or launcher repair.
---

# Adaptive operations runner

This is the operations instance of the adaptive-runner framework. A `tier`
proves an operational state rather than a test assertion. Read its `artifact:`
as the authoritative state probe named in the profile.

## Operations safety boundary

Classify the requested action before selecting a tier:

- **Safe and reversible:** inspect versions, processes, Docker Compose state,
  devices, disk space, health endpoints, and Git status; run
  `ОТЛАДКА-TIMA.bat` without switches; start an installed Docker Desktop, a
  requested stack, or an existing AVD only after proving no duplicate exists;
  use a root launcher for a requested TIMA target.
- **Approval required:** package/SDK downloads, UAC, WSL or Windows-feature
  changes, persistent environment variables, process termination, replacing an
  AVD, Docker rebuilds that replace runtime components, volume restore,
  deletion, firewall/network changes, Git push, or external credentialed work.
  Stop and request explicit approval, stating the retained-state risk.
- **Forbidden:** `docker compose down -v` / `down --volumes` during work that
  depends on state; wiping an AVD; `adb shell monkey` for TIMA; direct
  `TIMA.exe`, `go run ./cmd/tima`, or `go run ./cmd/escrow-stub` when a root
  launcher covers the target; reconstructing launcher environment variables by
  hand; altering global Git configuration; fabricating evidence.

Root launchers are authoritative runtime entry points. A failing launcher is
repaired through its source script using `Get-Command` to resolve tools, never
bypassed by starting the target manually. They are profile `run:` paths, not
candidate frozen scripts: do not copy, rename, or edit them as a worker setup
repair.
