...
Шина — Redis Pub/Sub (`REDIS_URL`, dev: `redis://:tima-dev-only@localhost:6379`); без него `/ws` отвечает 503. Ретеншен 90 дней + `sync.gap`, офлайн-очередь push (Redis Stream → FCM/APNs) — итерация worker-а.
