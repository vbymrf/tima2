...
Секрет JWT — `JWT_SIGNING_KEY`; без него генерируется эфемерный (dev). Rate limiting (Redis, `internal/ratelimit`, окно 10 мин): 3 SMS на телефон, 10 SMS с IP (первый `X-Forwarded-For` за Caddy), 5 попыток verify на `request_id` — перебор 6-значного кода отсекается; без `REDIS_URL` лимитов нет (dev). Ещё не реализовано из карты Auth: guest, refresh, recovery, `/link/*` (QR), attestation.
