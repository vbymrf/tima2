...
func (s *Server) reidentifyChallenge(w http.ResponseWriter, r *http.Request) {
	id, _ := auth.FromContext(r.Context())
	token, err := s.Auth.IssueReidentifyChallenge(id.UserID)
	if err != nil {
...
func (s *Server) reidentify(w http.ResponseWriter, r *http.Request) {
	...
	claims, err := s.Auth.Parse(req.ChallengeToken, auth.ScopeReidentify)
	...
	ctx := r.Context()
	personID, err := s.Store.PersonOfUser(ctx, id.UserID)
	if err != nil {
		log.Printf("reidentify: person: %v", err)
		writeErr(w, http.StatusInternalServerError, "internal", "ошибка хранилища")
		return
	}
	priorUserID, err := s.Store.FindPriorIdentity(ctx, personID, identityPub)
	...
	newUserID, err := s.Store.StartNewIdentity(ctx, personID, priorUserID, sig)
	if err != nil {
	...
	if err := s.Store.SetOrCheckIdentity(ctx, newUserID, identityPub); err != nil {
		log.Printf("reidentify: set identity: %v", err)
		writeErr(w, http.StatusInternalServerError, "internal", "ошибка хранилища")
		return
	}
	if err := s.Store.MoveDeviceToUser(ctx, id.DeviceID, newUserID); err != nil {
		log.Printf("reidentify: move device: %v", err)
		writeErr(w, http.StatusInternalServerError, "internal", "ошибка хранилища")
		return
	}
	access, err := s.Auth.IssueAccess(newUserID, id.DeviceID)
	if err != nil {
		writeErr(w, http.StatusInternalServerError, "internal", "не выдался токен")
		return
	}
