...
//	предел частоты      — по устройству
//
// Предел частоты по УСТРОЙСТВУ, а не по адресу: за одним адресом сидит подъезд, а
// перебирают каталог с одного аккаунта.
//
// У точного поиска (`lookupByNickname`) предела частоты НЕТ — ни сейчас, ни раньше.
// Это осознанное неравенство, а не недосмотр: точный отвечает про один полный ник
// (10…20 знаков), и перебирать им пространство имён втрое дороже, чем просто угадывать.
// Здесь же одна буква расширяет выдачу на десяток человек сразу.
...
// rlNickSearch — поисков по части ника с устройства за десятиминутное окно.
//
// Тридцать — это три поиска в минуту без перерыва: человеку, который ищет знакомого, с
// запасом; тому, кто снимает каталог по буквам, — ничто.
