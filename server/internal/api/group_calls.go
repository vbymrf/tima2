...
func joinCall(deps callsDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// ── РАЗРЕШЕНИЕ ОТКРЫТЫХ, А НЕ ПЕРЕЧИСЛЕНИЕ ЗАКРЫТЫХ ─────────────────
		//
		// Было `if state == "ended" || state == "missed"` — список похороненных. Он
		// уже протекал: `busy` в нём не значился, и звонок, закрытый с причиной
		// «занят», токен выдавал. С появлением `lost` (брошенный, закрыт уборщиком)
		// протёк бы второй раз — и молча.
		//
		// Перечисляем живые: звонок либо звонит, либо идёт. Всякое новое состояние
		// тогда по умолчанию закрыто, а не открыто, — и это единственная сторона, в
		// которую здесь можно ошибаться.
		if c.State != "ringing" && c.State != "answered" {
