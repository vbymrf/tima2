...
func (s *Server) Register(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/v1/users/discover", s.Auth.Require(s.discoverContacts))
	mux.HandleFunc("PATCH /api/v1/users/me/name", s.Auth.Require(s.setDisplayName))
	mux.HandleFunc("DELETE /api/v1/users/me", s.Auth.Require(s.deleteAccount))
	mux.HandleFunc("GET /api/v1/chats/archived", s.Auth.Require(s.listArchivedChats))
	mux.HandleFunc("PUT /api/v1/chats/{chatID}/archive", s.Auth.Require(s.setChatArchived))
	...
	mux.HandleFunc("POST /api/v1/link/confirm", s.Auth.Require(s.linkConfirm))
	mux.HandleFunc("POST /api/v1/link/claim", s.linkClaim)
	mux.HandleFunc("GET /api/v1/devices", s.Auth.Require(s.listMyDevices))
	mux.HandleFunc("PUT /api/v1/devices/me/platform", s.Auth.Require(s.setMyPlatform))
	mux.HandleFunc("DELETE /api/v1/devices/{deviceID}", s.Auth.Require(s.revokeDevice))
