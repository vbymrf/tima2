...
func (s *Server) Register(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/v1/chats/{chatID}/recover", s.requireActiveDevice(s.chatRecover))
	mux.HandleFunc("POST /api/v1/chats/{chatID}/recover/provide", s.requireActiveDevice(s.chatRecoverProvide))
	mux.HandleFunc("POST /api/v1/chats/{chatID}/backup", s.requireActiveDevice(s.chatBackupSave))
	mux.HandleFunc("GET /api/v1/chats/{chatID}/backup", s.requireActiveDevice(s.chatBackupList))
	...
	mux.HandleFunc("GET /api/v1/chats/archived", s.requireActiveDevice(s.listArchivedChats))
	mux.HandleFunc("PUT /api/v1/chats/{chatID}/archive", s.requireActiveDevice(s.setChatArchived))
	mux.HandleFunc("DELETE /api/v1/chats/{chatID}/archive", s.requireActiveDevice(s.setChatArchived))
