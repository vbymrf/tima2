...
func (s *Store) ResetForTests(ctx context.Context) error {
	_, err := s.pool.Exec(ctx, `TRUNCATE personal_messages, personal_message_keys, devices, users, sms_codes, media_objects, group_key_history, group_wrapped_keys, groups, memberships`)
	return err
}
